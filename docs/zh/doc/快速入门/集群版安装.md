...
如果是容器场景，需要将 127.0.0.1:8090，替换为 polaris-server http服务的address。如果您在node机器上执行命令，需要将其替换
为${pod_ip}:8090，其中pod_ip为statefulset中任意pod的ip。如果您在集群外执行，需要将其替换为polaris service对应的ip和http
服务对应的端口。
