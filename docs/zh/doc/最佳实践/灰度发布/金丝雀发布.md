...
金丝雀发布就是你已经将新版本应用部署到生产环境了，然后根据一定的特征，一般是用户ID、用户地域、用户类型等不同维度，允许部分用户使用部署到生产环境上的新版本应用。这个过程中，通过观察新版本的表现（比如成功率、时延、负载等），以判断新版本是否符合预期。当数据足以支撑全量发布时，就可以进行对新版本进行全量发布。
